namespace PrimeFactors
{
    public class Primes
    {
      ...
      //47, 43, 41, 37, 31, 29, 23, 19, 17, 13, 11, 7
       5, 3, 2
    };
        ...
        public static string PrimeFactors(int number)
        {
            ...
            foreach(int divisor in PrimeNumbers)
            {
                ...
                do
                {   
                    // step1. 10 / 5 remainder = 0
                    // step5. 2 / 3 remainder = 2
                    // step6. 2 / 2 remaider = 0                  
                    remainder = number % divisor;                     
                    if(remainder == 0)
                    {
                        // step2. number becomes 2 (10/5)
                        // step7. number becomes 1 (2/2)
                        number = number / divisor;
                        
                        if(number == 1)
                        {
                            //step8. add "2" in factors
                            factors += $"{divisor}";
                        }
                        // step3. number !=1, then go into this block
                        else
                        {
                            // step4. add "5 x" in factors 
                            factors += $"{divisor} x "; 
                        }
                    }
                } while (remainder == 0);              
